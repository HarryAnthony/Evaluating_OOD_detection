...
def get_ood_dataset(dataset_name):
    ...
    """
    if dataset_name not in ['SHVM', 'CIFAR10', 'CIFAR100']:
        raise ValueError('Dataset not supported. The dataset must be in the list [SHVM, cifar10, cifar1000].')
    
    if dataset_name in ['SHVM','CIFAR10','CIFAR100']:
        if dataset_name == 'SHVM':   
            from source.config import SHVM as ood_cf
            ood_dataset = torchvision.datasets.SVHN(root='data/SHVM', split='test', download=True, transform=ood_cf.transform_test)
        elif dataset_name == 'CIFAR10':
            from source.config import cifar10 as ood_cf
            ...
            from source.config import cifar100 as ood_cf
