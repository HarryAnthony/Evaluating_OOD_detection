...
class Train_DNN():
    ...
    def __init__(self, training_dict, **kwargs):
        ...
        self.wandb_dict = training_dict['wandb_dict']
    ...
    def log_training_info(self,**kwargs):  
        ...
        if self.wandb_dict['bool']:
            self.wandb_dict['wandb'].log({"epoch": self.epoch})
            self.wandb_dict['wandb'].log({"lr": self.scheduler.get_last_lr()[0]})
    ...
    def log_training_epoch_progress(self,**kwargs):
        ...
        if self.wandb_dict['bool']:
            self.wandb_dict['wandb'].log({"train_loss": self.loss_mean})
            self.wandb_dict['wandb'].log({"train_acc": self.accuracy})
    ...
    def log_validation_epoch_progress(self,**kwargs):
        ...
        if self.verbose:    
            print('\n| Validation \tLoss: %.4f \tAcc@1: %.3f%%'
                                % (self.loss_mean, self.accuracy))
        if self.wandb_dict['bool']:
            self.wandb_dict['wandb'].log({"val_loss": self.loss_mean})
            self.wandb_dict['wandb'].log({"val_acc": self.accuracy})
